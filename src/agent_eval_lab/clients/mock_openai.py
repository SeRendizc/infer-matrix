...
from agent_eval_lab.clients.base import BaseClient


class MockOpenAIClient(BaseClient):
    """一个假的 OpenAI-compatible client。

    它的职责是根据 EvalCase 生成稳定、可预测的 mock response。
    ...
    def stream_case(self, case: EvalCase) -> list[dict[str, Any]]:
        """运行一个 streaming case，并返回 mock streaming chunks。

        Args:
            case: features.streaming=True 的 EvalCase。
    ...
    def _build_text_response(self, case: EvalCase) -> dict[str, Any]:
    ...
    def _build_tool_call_response(self, case: EvalCase) -> dict[str, Any]:
    ...
    def _build_text_content(self, case: EvalCase) -> str:
        """构造普通文本回复内容。"""
        ...
        return "Agent Eval Lab is an Agentic LLM Systems behavior analysis framework."

    def _get_stream_content_chunks(self, case: EvalCase) -> list[str]:
    ...
    def _optional_metadata_string(self, case: EvalCase, key: str) -> str | None:
