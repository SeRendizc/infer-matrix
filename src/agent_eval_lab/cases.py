"""Case definitions and loader for Agent Eval Lab.
...
class ProtocolConfig(BaseModel):
    """Agent Eval Lab 与 Backend 交互时使用的 API 协议。
...
def load_case(path: str | Path) -> EvalCase:
    """从 YAML 文件读取一个 Agent Eval Lab case。

    Args:
        path: YAML case 文件路径。

    Returns:
        EvalCase: 通过 Pydantic 校验后的 case 对象。

    Raises:
        FileNotFoundError: case 文件不存在。
        ValueError: case 文件为空。
        pydantic.ValidationError: YAML 内容不符合 EvalCase schema。
    """
