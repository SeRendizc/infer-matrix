"""Run report assembler for Agent Eval Lab."""
...
def _validate_run_matches_case(
    *,
    ...
    """确认 RunResult 确实属于当前 EvalCase。"""

    if run_result.case_id != case.case_id:
        raise ReportAssemblyError(
            "RunResult case_id does not match EvalCase: "
    ...
    if (
        ...
        raise ReportAssemblyError(
            "RunResult backend does not match EvalCase: "
    ...
    if run_result.model != case.model:
        raise ReportAssemblyError(
            "RunResult model does not match EvalCase: "
    ...
    if (
        ...
        raise ReportAssemblyError(
            "RunResult protocol does not match EvalCase: "
